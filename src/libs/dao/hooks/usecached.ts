...
//   function Model(variables:{modelName:string}) { 
//     const result = client.readQuery({ 
//       query: QUERY_MODEL, variables 
//     }); 
...
//     const {ifields} = Model({modelName}); 
//     return fieldNames.filter( name => ifields.some(f => f.accessor === name) ) 
//       .map( name => ifields.find(f => f.name === name)) as IField[]; 
//   }
